...
(function ($) {
  ...
  $.fn.zTree = {
    ...
    init: function (obj, zSetting, zNodes, customSetting) {
      var setting = tools.clone(_setting);
      $.extend(true, setting, customSetting, zSetting);
...
(function ($) {
  ...
  TreeSelect.prototype.render = function (options) {
    var elem = options.elem,
        ...
        customSetting = options.ztree,
        checkedId = options.checked,
    ...
    var a = {
      init: function(){
        $.ajax({
          ...
          success: function (d) {
        	  debugger;
        	d = loadFilter&&typeof loadFilter == 'function'?loadFilter(d):d;
            DATA = d;
            a.hideElem().input().toggleSelect().loadCss().preventEvent();
            var $ztree = $.fn.zTree.init($('#' + TREE_SELECT_BODY_ID), a.setting(), d, customSetting);
            /*if(checkedId!=null && checkedId!=''){
            	var node = $ztree.getNodeByParam(idName, checkedId);
            	if(node!=null) {
      ...
      onClick: function(event, treeId, treeNode){
        var id = treeNode.id,
            ...
            zTreeObj = this.getZTreeObj(treeId),
            name = treeNode[zTreeObj.setting.data.key.name];
        $input.val(name);
