layui.use(['form','layer','table','laytpl','cjs'],function(){
    ...
    var tableIns = table.render($.extend(cjs.layui.table, {
        ...
        cols : [[
            ...
            {field: 'username', title: '微信昵称', minWidth:100, align:"center"},
            {field: 'createtime', title: '授权时间' ,minWidth:150, align:'center'},
            {field: 'updateTime', title: '最后登录时间' ,minWidth:150, align:'center'},
        ]]
    }));
	...
	function userInfoView(uid) {
		cjs.layer.full({
			title : "添加用户",
			type : 2,
			content : "/admin/user/user?uid="+(uid?uid:""),
    ...
    $(".delBtn").click(function(){
		...
		if(data.length == 1){
			var index = layer.confirm('确定删除选中的用户？', {icon: 3, title: '提示信息'}, function (index) {
				$.ajax({
					url : '/admin/user/delete?uid='+data[0].uid,
					success : function(d) {
